...
void ClassAssistant::pub_summary(PubSubClient client)
{
    String data_str = class_type + ":"
            + String(students) + ":"
            + time_type + ":"
            + day_type+ ":"
    ...
    const char* topic = "class_summary";

    client.publish(topic, data_str.c_str());
}
...
//     rtc.adjust(now);
//     now = rtc.now();
//     nameFile = "class_data_" + print_time(now);
//     String txt = ".txt";
